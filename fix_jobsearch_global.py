...
# First, we remove the previously injected dark mode completely
html = re.sub(r'<!-- INJECTED DARK MODE -->.*?<!-- END INJECTED DARK MODE -->', '', html, flags=re.DOTALL)
html = re.sub(r'<!-- GLOBAL FIXES -->.*?<!-- END GLOBAL FIXES -->', '', html, flags=re.DOTALL)

global_css = '''
<!-- GLOBAL FIXES -->
<style>
    /* Fix overlap of logo and save button for ALL modes */
    .sol-card-header {
...
with open(html_path, 'w', encoding='utf-8') as f:
    f.write(html)
print("Injected global fix for overlapping save button")
