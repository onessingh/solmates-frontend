...
function uiShowCreateRoom() {
    if (!localStorage.getItem('solmates_nickname')) { openProfileModal(); return; } hideAllScreens(); document.getElementById('screen-create').classList.remove('hidden'); populateSemesters(); }
...
function manualJoinRoom() {
    ...
    const failTimer = setTimeout(() => { statusEl.textContent = "Could not connect."; showToast("Connection timed out."); }, 40000);
...
function handleGuestData(data) {
    ...
    if (data.type === 'QUESTION') { 
        ...
        gameState.questionReceived = true; gameState.qIndex = data.qIndex; 
        ...
        showQuestion(data.qIndex, data.question); 
    }
...
function enterGameFromAuthoritativeState(sourceData, sourceName) {
    ...
    if (!gameState.gameStarted) {
        ...
        if (!gameState.questions && sourceData.questions) {
            gameState.questions = sourceData.questions;
        }
    }
}
