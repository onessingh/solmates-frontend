...
if parth_match:
    ...
    info_inner = parth_match.group(4)
...
if prince_match:
    ...
    info_inner = prince_match.group(4)
...
if parvesh_match:
    ...
    info_inner = parvesh_match.group(4)
...
# 5. Vikas Bal
vikas_pattern = re.compile(r'(<div class="member-image">.*?<div class="image-fallback" id="fallback-1">Vikas Bal</div>\s*</div>)\s*(<div class="member-info">.*?)(</div>\s*</div>)', re.DOTALL)
vikas_match = vikas_pattern.search(content)
if vikas_match:
    img_block = vikas_match.group(1)
    info_inner = vikas_match.group(2)
