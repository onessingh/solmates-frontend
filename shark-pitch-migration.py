...
with open('games/shark-pitch/game.js', 'r', encoding='utf-8') as f:
    js = f.read()

# 1. Add migrateHost function at the end
migrate_func = '''
function migrateHost(hostId) {
    if (!gameState.challenges || gameState.challenges.length === 0) return;
...
# Shark Pitch merges settings too!
# if (data.type === 'LOBBY_UPDATE') { ... gameState = { ...gameState, ...data.settings }; ... }

with open('games/shark-pitch/game.js', 'w', encoding='utf-8') as f:
    f.write(js)
