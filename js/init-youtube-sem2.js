document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.loadYouTubeVideos) await loadYouTubeVideos('2', container.id);
  ...
  if (typeof io !== 'undefined') {
    try {
      ...
      _sock.emit('sol:subscribe', { category: 'youtube', semester: '2' });
      _sock.on('reconnect', function() {
        _sock.emit('sol:subscribe', { category: 'youtube', semester: '2' });
      });
      _sock.on('sol:updated', async function(payload) {
        if (payload.category === 'youtube' && String(payload.semester) === '2') {
          var _cid = document.getElementById('videoContainer') ? 'videoContainer' : 'contentContainer';
          if (window.loadYouTubeVideos) await loadYouTubeVideos('2', _cid);
        }
      });
    ...
    } catch(e) { console.warn('Socket init failed (init-youtube-sem2):', e); }
  }
