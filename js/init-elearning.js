/**
 * Elearning Page Initialization
 * Dynamically loads elearning content from backend
 */
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.loadContent && typeof window.loadContent === 'function') {
    await loadContent('elearning', semester, 'contentContainer');
  } else {
  ...
  if (typeof io !== 'undefined') {
    try {
      ...
      _sock.on('content:updated', async function(payload) {
        if (payload.type === 'elearning') {
          var _p = new URLSearchParams(window.location.search);
          var _sem = _p.get('semester');
          if (window.loadContent) await loadContent('elearning', _sem, 'contentContainer');
        }
      });
    } catch(e) { console.warn('Socket init failed (init-elearning):', e); }
  }
  ...
  setInterval(async () => {
    if (window.loadContent) {
      var _p = new URLSearchParams(window.location.search);
      await loadContent('elearning', _p.get('semester'), 'contentContainer');
    }
  }, 15000);
});
