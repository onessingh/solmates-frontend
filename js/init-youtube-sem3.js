document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.loadYouTubeVideos) await loadYouTubeVideos('3', container.id);
  ...
  if (typeof io !== 'undefined') {
    try {
      ...
      _sock.emit('sol:subscribe', { category: 'youtube', semester: '3' });
      _sock.on('reconnect', function() {
        _sock.emit('sol:subscribe', { category: 'youtube', semester: '3' });
      });
      _sock.on('sol:updated', async function(payload) {
        if (payload.category === 'youtube' && String(payload.semester) === '3') {
          var _cid = document.getElementById('videoContainer') ? 'videoContainer' : 'contentContainer';
          if (window.loadYouTubeVideos) await loadYouTubeVideos('3', _cid);
        }
      });
    ...
    } catch(e) { console.warn('Socket init failed (init-youtube-sem3):', e); }
  }
  ...
  setInterval(async () => {
    if (window.loadYouTubeVideos) {
      var _cid = document.getElementById('videoContainer') ? 'videoContainer' : 'contentContainer';
      await loadYouTubeVideos('3', _cid);
    }
  }, 15000);
});
