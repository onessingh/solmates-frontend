/**
 * Professor Page Initialization
 * Dynamically loads professor content from backend
 */
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.loadContent && typeof window.loadContent === 'function') {
    await loadContent('professor', semester, 'contentContainer');
  } else {
    ...
    try {
      ...
      _sock.on('content:updated', async function(payload) {
        if (payload.type === 'professor') {
          ...
          if (window.loadContent) await loadContent('professor', _sem, 'contentContainer');
        }
      });
    } catch(e) { console.warn('Socket init failed (init-professor):', e); }
  }
