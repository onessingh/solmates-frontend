/**
 * Pyq Page Initialization
 * Dynamically loads pyq content from backend
 */
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.loadContent && typeof window.loadContent === 'function') {
    await loadContent('pyq', semester, 'contentContainer');
  } else {
    ...
    try {
      ...
      _sock.on('content:updated', async function(payload) {
        if (payload.type === 'pyq') {
          ...
          if (window.loadContent) await loadContent('pyq', _sem, 'contentContainer');
        }
      });
    } catch(e) { console.warn('Socket init failed (init-pyq):', e); }
  }
