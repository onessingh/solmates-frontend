...
# Remove the old custom block
content = re.sub(r'/\* CUSTOM DARK MODE FOR RESUME BUILDER \*/[\s\S]*', '', content)
...
@media (prefers-color-scheme: dark) {
    ...
    /* Interface Cards */
    .card, .form-panel, .panel-section, .hero, .rb-tab-content, .white-box, .metric, .template-card, .rb-tabs, .wizard-header { 
    ...
    .eyebrow, .hero-subtitle, p, label, .form-label, .wizard-step-text, .metric-label, .section-header p { color: #94a3b8 !important; }
    ...
    /* JD Match specific */
    .jd-match, .jd-card, .score-card { background: #1e293b !important; border: 1px solid #334155 !important; }
...
with open(html_path, 'r', encoding='utf-8') as f:
    html = f.read()
html = re.sub(r'style\.css\?v=[0-9.]+', 'style.css?v=111.7', html)
